...
from server.db import CameraModel, DeviceType, Station, StationService, NotFoundError
...
@no_type_check
def test_insert_and_get_station_sucess(client: FlaskClient) -> None:
    station = Station(
...
@no_type_check
def test_list_stations_unsuccessful(client: FlaskClient) -> None:
    stations = StationService.list()
    assert isinstance(stations, list), "Expected a list of stations"
    assert len(stations) == 0, f"Expected 0 stations, got {len(stations)}"
...
@no_type_check
def test_update_station_successful(client: FlaskClient) -> None:
    station = Station(
        MAC="00:1A:2B:3C:4D:5E",
        name="station",
        device_model=DeviceType.ESP32,
...
@no_type_check
def test_update_station_nonexistant(client: FlaskClient) -> None:
    MAC = "00:1A:2B:3C:4D:5E"
    updates = {"name": "updated_station", "device_model": DeviceType.ESP8266}

    try:
        StationService.update(MAC=MAC, station=updates)
        raise AssertionError("updating nonexistant station should raise NotFOundError")
    except NotFoundError as nfr:
        pass


@no_type_check
def test_delete_station_successful(client: FlaskClient) -> None:
    station = Station(
        MAC="00:1A:2B:3C:4D:5E",
        name="station",
        device_model=DeviceType.ESP32,
...
@no_type_check
def test_delete_station_nonexistant(client: FlaskClient) -> None:
    MAC = "00:1A:2B:3C:4D:5E"
    station = StationService.get(MAC=MAC)
    assert station is None, f"Station should not exist, should be None, got {station}"

    try:
        StationService.delete(MAC=MAC)
        raise AssertionError(
            "Deleting nonexstent sation should have raised NotFoundError"
        )
...
@no_type_check
def test_station_exists_true(client: FlaskClient) -> None:
    station = Station(
        MAC="00:1A:2B:3C:4D:5E",
        name="station",
        device_model=DeviceType.ESP32,
...
@no_type_check
def test_station_exists_false(client: FlaskClient) -> None:
    MAC = "00:1A:2B:3C:4D:5E"
    exists = StationService.exists(MAC=MAC)
    assert exists == False, "Station should exist after insertion"


@no_type_check
def test_station_exists_after_delete(client: FlaskClient) -> None:
    station = Station(
        MAC="00:1A:2B:3C:4D:5E",
        name="station",
        device_model=DeviceType.ESP32,

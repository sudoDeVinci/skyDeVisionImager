...
def _vectorized_xyz_extraction(longitudes, latitudes, raster_data):
...
def meshify(
    ...
    @njit(
        types.Tuple((
            types.npytypes.Array(types.float64, 1, "C"),
            types.npytypes.Array(types.float64, 1, "C"),
            types.npytypes.Array(types.float64, 1, "C"),
    ...
    def data_prepare(xyz: GeoCoordinateArray, points: uint16) -> tuple[NDArray[Shape["*"], Float32], ...]:
        ...
        return x_grid, y_grid, z_scaled, x, y
...
def graph_plotly(
    ...
    if meshes is None:
        ...
        meshes = meshify(xyz, points=points, scalar=1.0, interpolation="nearest", smooth_sigma=1.0)
